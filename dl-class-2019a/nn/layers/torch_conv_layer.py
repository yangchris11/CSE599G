...
class TorchConvLayer(Layer):
    ...
    def forward(self, data):
        ...
        self.output = F.conv2d(self.data.double(), self.weight_tensor.double(), self.bias_tensor.double(), self.stride, self.padding)
        return utils.to_numpy(self.output)

    def backward(self, previous_partial_gradient):
        gradients = utils.from_numpy(previous_partial_gradient)
        input_grad = grad.conv2d_input(self.data.shape, self.weight_tensor.double(), gradients, self.stride, self.padding)
        weight_grad = grad.conv2d_weight(self.data.double(), self.weight_tensor.shape, gradients, self.stride, self.padding)
